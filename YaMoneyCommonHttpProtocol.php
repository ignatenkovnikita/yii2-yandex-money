...
class YaMoneyCommonHttpProtocol {
    ...
    public function __construct($action, \stdClass $settings) {
        $this->action = $action;
    ...
    public function processRequest($request) {
        ...
        if ($this->settings->SECURITY_TYPE == "MD5") {
            ...
            if (!$this->checkMD5($request)) {
                $response = $this->buildResponse($this->action, (isset($request['invoiceId']) ? $request['invoiceId'] : null), 1);
    ...
    public function checkMD5($request) {
        if (
            ...
            || !isset($request['orderSumAmount'])
            || !isset($request['orderSumCurrencyPaycash'])
            || !isset($request['orderSumBankPaycash'])
            || !isset($request['shopId'])
            || !isset($request['invoiceId'])
            || !isset($request['customerNumber'])
        ) {
            return false;
        }
